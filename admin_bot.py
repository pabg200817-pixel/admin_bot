...
FUCKUP_PHRASES = [
    "Ты че исполняешь? Сделай нормально!",
    "Ну че за хуйню ты скинул? Глаза разуй!",
    "Опять пальцы кривые? Читай че на экране написано!",
    "Всё хуйня, давай по новой!",
    "Ты че, дурак? Нахуя ты мне это шлешь?"
]
...
async def cmd_start(message: Message, state: FSMContext):
    await state.clear()
    runtime_config["accounts_processed"] = 0
    await message.answer("⚙️ Бот готов. Шаг 1: Напиши название для новых групп:", reply_markup=get_admin_keyboard())
    await state.set_state(SetupPipeline.waiting_for_name)
...
async def process_photo(message: Message, state: FSMContext):
    ...
    await message.answer("👥 Шаг 3: Скинь список НОМЕРОВ ТЕЛЕФОНОВ (по одному в строке):")
...
async def process_phone(message: Message, state: FSMContext):
    ...
    if await client.is_user_authorized():
        await message.answer("✅ Аккаунт авторизован. Делаю группы...")
        await state.clear()
...
async def process_tg_code(message: Message, state: FSMContext):
    try:
        ...
        await message.answer("🔒 Нужен пароль облачного пароля:")
